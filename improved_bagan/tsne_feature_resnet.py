...
def tsne_plot(encoder):
    ...
    for x, y, l in [(x_real, y_real, real_label), (x_generated, y_generated, generated_label)]:
        marker = markers[loop]
        color_label = color_label[loop]
        message_label = message_label[loop]
        loop += 1
        #plt.scatter(x, y, marker=marker, c=color_label, label=message_label)
        for c in range(n_classes):
            plt.scatter(x[l == c], y[l == c], marker=marker, c=np.array([color(c)]), label='%d' % c)
    plt.legend()
    plt.savefig('bagan_gp_results/tsne_feature_resnet.png')
    plt.show()
